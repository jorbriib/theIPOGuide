...
func TestHydrateIpo(t *testing.T) {
	...
	now := time.Now()
	ipo := HydrateIpo("uuid",  "alias", "uuid", "uuid", 3222, 3444, 10029039, &now)
...
func TestIpo_Id(t *testing.T) {
	...
	now := time.Now()
	ipo := HydrateIpo("29392-32929da", "alias", "uuid", "uuid", 3222, 3444, 10029039, &now)
...
func TestIpo_CompanyId(t *testing.T) {
	...
	now := time.Now()
	ipo := HydrateIpo("29392-32929da", "alias", "m-uuid", "c-uuid", 3222, 3444, 10029039, &now)
